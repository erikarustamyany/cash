...
<div class="store-filter clearfix" style="margin-top: 20px;">
    <div class="pull-left">
    <?php  if($isTop) { ?>
        <div class="row-filter hidden-on-phone">
            <button class="change-list-type <?= (!empty($_SESSION['list-type']) && $_SESSION['list-type'] == 'inline')? 'active':'' ?>" data-type="inline"><i class="fa fa-th-large"></i></button>
            <button class="change-list-type <?= (empty($_SESSION['list-type']) || (!empty($_SESSION['list-type']) && $_SESSION['list-type'] == 'block'))? 'active':'' ?>" data-type="block"><i class="fa fa-bars"></i></button>
        </div>
